...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        Block.
        """
        # set the position of this block to <position>
        self.position = position

        # change the position of its children
        if len(self.children) != 0:
            for i in range(4):
                self.children[i].position = self._children_positions()[i]

        # change the position of its descendants
        for child in self.children:
    ...
    def smash(self) -> bool:
        ...
        """
        # if the Block cannot be smashed
        if not self.smashable():
            return False

        # a Block with children has no colour
        self.colour = None

        # randomly generate 4 children for the block
        self._generate_children()

        for child in self.children:
            is_picked = random.random() < math.exp(-0.25 * self.level)
            if child.smashable() and is_picked:
                child.smash()
            else:
                child.colour = COLOUR_LIST[random.randrange(0, 4)]

        return True

    def _generate_children(self) -> None:
        """Generate 4 random children Block for the given parent Block.

        >>> block = Block((0, 0), 10, COLOUR_LIST[0], 0, 2)
        >>> len(block.children)
        0
        >>> block._generate_children()
        >>> len(block.children)
        4
        """
        # a Block with children has no colour
        ...
        for i in range(4):
            ...
            colour = COLOUR_LIST[random.randrange(4)]
            self.children.append(Block(position, size, colour,
                                       level, max_depth))
    ...
    def swap(self, direction: int) -> bool:
        ...
        # swap vertically
        if direction == 1:
            # self.children = [child[3], child[2], child[1], child[0]]
            self.children.reverse()

        # swap horizontally
        else:
            self.children = [child[1], child[0], child[3], child[2]]

        # does all descendants need to be swapped?
    ...
    def rotate(self, direction: int) -> bool:
        ...
        # rotate clockwise
        if direction == 1:
            self.children = [child[1], child[2], child[3], child[0]]
        # rotate counterclockwise
        else:
            self.children = [child[3], child[0], child[1], child[2]]
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        # if it is not a leaf or it is in the same colour as assigned
        if self.level != self.max_depth or self.colour == colour:
            return False

        # else, change the colour and return True
        self.colour = colour
        return True
    ...
    def combine(self) -> bool:
        ...
        """
        # if the block has no children or is not at level of max_depth - 1
        ...
        for child in self.children:
            ...
            if child_colour not in colours:
                colours[child_colour] = 0
            colours[child_colour] += 1

        # find the major colour
        highest_count = 0
        major_colour = (0, 0, 0)
        for colour in colours:
            if colours[colour] > highest_count:
                highest_count = colours[colour]
                major_colour = colour

        # check to make sure there is no tie situation
        for colour in colours:
            if colour != major_colour and colours[colour] == highest_count:
                return False

        self.children = []
        self.colour = major_colour
        return True
    ...
    def create_copy(self) -> Block:
        ...
        if len(self.children) == 0:
            ...
            new_children = []
            for child in self.children:
                new_children.append(child.create_copy())
